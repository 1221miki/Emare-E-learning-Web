...
const liveBadgeClass = (live) => ({
    upcoming: 'border-blue-400/40 bg-black/50 text-blue-300',
...
}[live] || 'border-blue-400/40 bg-black/50 text-blue-300');
        ...
        <div className="flex flex-wrap items-center gap-3">
            {cells.map((c, i) => (
                <React.Fragment key={c.label}>
                    <div className="flex h-[74px] w-[74px] flex-col items-center justify-center rounded-2xl border border-blue-500/25 bg-[#1A1B23]/90 shadow-[0_0_28px_rgba(59,130,246,0.15)] backdrop-blur">
                        <span className="text-2xl font-black tabular-nums text-white">{c.value}</span>
                        <span className="mt-0.5 text-[9px] font-bold tracking-[0.22em] text-blue-400">{c.label}</span>
                    </div>
                    {i < cells.length - 1 && <span className="text-xl font-black text-blue-400/60">:</span>}
                </React.Fragment>
            ))}
        </div>
    );
        ...
        <div className="relative min-h-screen overflow-x-hidden bg-[linear-gradient(135deg,#0B0C10_0%,#14141F_45%,#1F1F2E_100%)] text-white">
            ...
            <main className="relative z-10 mx-auto max-w-7xl px-4 pt-24 sm:px-6 sm:pt-28">
                <div className="mb-8 text-center">
                    <span className="text-xs font-extrabold uppercase tracking-[0.25em] text-blue-400">Upcoming Events</span>
                    <h1 className="mt-3 text-3xl font-black tracking-tight sm:text-5xl">
                        Learn. Create. <span className="bg-gradient-to-r from-blue-300 to-violet-400 bg-clip-text text-transparent">Grow.</span>
                    </h1>
                ...
                {loadState === 'loading' ? (
                    <div className="rounded-3xl border border-blue-500/20 bg-[#12131A]/80 px-6 py-20 text-center">
                    ...
                    <div className="rounded-3xl border border-red-400/30 bg-[#12131A]/80 px-6 py-20 text-center">
                        ...
                        <button
                            onClick={loadEvents}
                            className="mt-6 inline-flex items-center gap-2 rounded-2xl bg-gradient-to-r from-blue-400 to-violet-500 px-6 py-3 text-sm font-extrabold uppercase tracking-wide text-black transition hover:brightness-110"
                        >
                            Retry
                        </button>
                    </div>
                    ...
                    <>
                {/* ── Featured Event Hero ─────────────────────────────────── */}
                <section className="relative overflow-hidden rounded-3xl border border-blue-500/20">
                    ...
                    <div className="relative flex flex-col gap-6 px-6 py-12 sm:px-12 sm:py-16">
                        <div>
                            ...
                            <h2 className="mt-5 max-w-3xl text-4xl font-black leading-[1.05] tracking-tight sm:text-6xl">
                                {featured.title.split('&').map((part, i) =>
                                    i === 0 ? part : (
                                        <span key={i} className="bg-gradient-to-r from-blue-300 to-violet-400 bg-clip-text text-transparent">&amp;{part}</span>
                                    )
                                )}
                            </h2>
                        </div>
                        ...
                        <div className="flex flex-wrap items-center gap-x-6 gap-y-3 text-sm text-gray-300">
                            <span className="flex items-center gap-2"><Calendar className="h-4 w-4 text-blue-400" /> {formatISODate(featured.date)}</span>
                            <span className="flex items-center gap-2"><Clock className="h-4 w-4 text-blue-400" /> {featured.time}</span>
                            <span className="flex items-center gap-2"><MapPin className="h-4 w-4 text-blue-400" /> {featured.location}</span>
                        </div>
                        ...
                        <div className="mt-2 flex flex-col gap-5 border-t border-white/10 pt-6 sm:flex-row sm:items-center sm:justify-between">
                            <Link
                                ...
                                className="inline-flex items-center justify-center gap-2 rounded-2xl bg-gradient-to-r from-blue-400 to-violet-500 px-8 py-4 text-sm font-extrabold uppercase tracking-wide text-black shadow-[0_0_30px_rgba(255,193,7,0.45)] transition hover:shadow-[0_0_45px_rgba(255,193,7,0.6)]"
                            >
                                View Event Details <ArrowRight className="h-4 w-4" />
                            </Link>
                            <div className="w-full sm:w-64">
                                <div className="mb-1.5 flex items-center justify-between text-xs">
                                    <span className="font-semibold text-blue-300">👥 {featured.slotsLeft} Slots Left</span>
                                    <span className="text-gray-400">{featured.price === 'FREE' ? 'Free' : featured.price}</span>
                                </div>
                                <div className="h-2.5 overflow-hidden rounded-full bg-white/10">
                                    <div
                                        className="h-full rounded-full bg-gradient-to-r from-blue-400 to-violet-500 shadow-[0_0_12px_rgba(255,193,7,0.6)]"
                ...
                <section className="mt-20">
                    <div className="mb-6 flex items-center gap-3">
                        <span className="text-xs font-extrabold uppercase tracking-[0.25em] text-blue-400">All Events</span>
                    ...
                    <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
                        {others.map((event) => (
                            <Link
                                key={event.id}
                                to={`/events/${event.id}`}
                                className="group flex flex-col overflow-hidden rounded-3xl border border-blue-500/20 bg-[#12131A] transition hover:border-blue-400/50 hover:shadow-[0_0_35px_rgba(255,193,7,0.15)]"
                            >
                                ...
                                <div className="flex flex-1 flex-col p-5">
                                    <h3 className="text-lg font-extrabold text-white transition group-hover:text-blue-300">{event.title}</h3>
                                    <p className="mt-1 text-xs text-gray-400">{event.tagline}</p>
                                    <div className="mt-4 space-y-2 text-xs text-gray-300">
                                        <p className="flex items-center gap-2"><Calendar className="h-3.5 w-3.5 text-blue-400" /> {formatISODate(event.date)}</p>
                                        <p className="flex items-center gap-2"><Clock className="h-3.5 w-3.5 text-blue-400" /> {event.time}</p>
                                        <p className="flex items-center gap-2"><MapPin className="h-3.5 w-3.5 text-blue-400" /> {event.location}</p>
                                    </div>
                                    <div className="mt-4 flex items-center justify-between border-t border-white/5 pt-4">
                                        ...
                                        <span className="flex items-center gap-1.5 text-xs font-bold text-gray-200">
                                            <Tag className="h-3.5 w-3.5 text-blue-400" /> {event.price}
                                        </span>
                                    </div>
                                    <span className="mt-4 text-xs font-bold text-blue-400 opacity-0 transition group-hover:opacity-100">
                                        View Details →
                                    </span>
                                </div>
                            </Link>
