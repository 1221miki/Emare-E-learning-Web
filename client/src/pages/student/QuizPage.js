...
export default function QuizPage() {
    ...
    if (loading) return <div style={styles.centerContainer}>Loading Quiz...</div>;
    if (!quiz) return <div style={styles.centerContainer}>Quiz not found.</div>;

    if (result) {
        return (
            <div style={styles.centerContainer}>
                <div style={styles.resultCard}>
                    <h2 style={{margin:'0 0 8px', color:'#f1f5f9'}}>Quiz Results</h2>
        ...
        <div style={styles.page}>
            <header style={styles.header}>
                <div>
                    ...
                    <p style={styles.subtitle}>Answer all questions before the timer expires.</p>
                </div>
                                ...
                                {q.options.map((opt, optIndex) => {
                                    ...
                                    return (
                                        ...
                                        >
                                            ...
                                            <span style={{color: isSelected ? '#fff' : '#cbd5e1'}}>{opt}</span>
                                        </div>
                                    );
                                })}
                            </div>
...
const styles = {
    centerContainer: { display: 'flex', alignItems: 'center', justifyContent: 'center', minHeight: '100vh', background: '#0f172a', fontFamily: "'Segoe UI', sans-serif" },
    resultCard: { background: '#1e293b', padding: '48px', borderRadius: '24px', border: '1px solid #334155', textAlign: 'center', maxWidth: '400px', width: '100%' },
    ...
    page: { minHeight: '100vh', background: '#0f172a', fontFamily: "'Segoe UI', sans-serif", paddingBottom: '80px' },
    header: { background: '#1e293b', borderBottom: '1px solid #334155', padding: '24px 40px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', position: 'sticky', top: 0, zIndex: 10 },
    title: { color: '#f1f5f9', fontSize: '24px', fontWeight: '800', margin: '0 0 4px' },
    ...
    questionCard: { background: '#1e293b', border: '1px solid #334155', borderRadius: '16px', padding: '32px', marginBottom: '24px' },
    questionText: { color: '#f1f5f9', fontSize: '18px', fontWeight: '600', margin: '0 0 24px', lineHeight: '1.5', display: 'flex', gap: '12px', alignItems: 'flex-start' },
    qNum: { background: 'rgba(255,255,255,0.1)', color: '#94a3b8', padding: '2px 8px', borderRadius: '6px', fontSize: '14px', flexShrink: 0 },
